...
[CmdletBinding()]
param(
    [string]$AccountPath = "$env:APPDATA\.feather\account.txt",
    [string]$LocalStatePath = "$env:APPDATA\Feather Launcher\Local State",
    [string]$OutputPath = "$env:APPDATA\.feather\account_decrypted.txt"
)
...
try {
    ...
    Write-Host "Reading encrypted account data..." -ForegroundColor Cyan
